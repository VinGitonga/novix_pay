...
import { ArrowLeft, CreditCard, DollarSign, CheckCircle, XCircle, Loader2, ExternalLink, Copy, Wallet } from "lucide-react";
...
const PaymentLinkCheckout = () => {
	...
	const handlePayment = async () => {
		...
		try {
			...
			const paymentRequirements = selectPaymentRequirements(state.paymentLink.paymentRequirements, "etherlink-testnet", "exact");
	...
	const isExpired = state.paymentLink?.expiresAt ? new Date(state.paymentLink.expiresAt) < new Date() : false;
				...
				<Card className="w-full max-w-md">
					<CardContent className="pt-6">
						<div className="text-center">
							...
							<p className="text-muted-foreground mb-4">{state.error || "This payment link doesn't exist or has been removed."}</p>
					...
					<CardContent className="pt-6">
						<div className="text-center">
							...
							<div className="space-y-2 mb-6">
								<p className="text-sm text-muted-foreground">Amount: {formatAmount(state.paymentLink.amount)}</p>
								<p className="text-sm text-muted-foreground">Business: {state.paymentLink.businessName}</p>
							</div>
							<Button onClick={() => navigate("/")}>
								<ArrowLeft className="h-4 w-4 mr-2" />
								Return Home
				...
				<div className="mb-8">
					...
					<div className="flex items-center justify-between">
						<div className="flex items-center space-x-3">
							{state.paymentLink.businessLogo && <img src={state.paymentLink.businessLogo} alt={state.paymentLink.businessName} className="w-12 h-12 rounded-lg" />}
							<div>
								<h1 className="text-2xl font-bold text-gray-900">{state.paymentLink.businessName}</h1>
							...
							<CardContent className="space-y-4">
								{state.paymentLink.productName && (
									<div className="flex items-center space-x-4">
										{state.paymentLink.productImage && <img src={state.paymentLink.productImage} alt={state.paymentLink.productName} className="w-16 h-16 rounded-lg object-cover" />}
										<div>
											<h3 className="font-semibold">{state.paymentLink.productName}</h3>
											<p className="text-sm text-muted-foreground">{state.paymentLink.description}</p>
										</div>
									</div>
								)}
								...
								<div className="space-y-3">
									<div className="flex justify-between">
										...
										<span className="font-semibold text-lg">{formatAmount(state.paymentLink.amount)}</span>
									</div>
									<div className="flex justify-between">
									...
									{state.paymentLink.expiresAt && (
										<div className="flex justify-between">
											...
											<span className="text-sm">{new Date(state.paymentLink.expiresAt).toLocaleDateString()}</span>
										</div>
									)}
								</div>
								...
								{isExpired && (
									<div className="bg-red-50 border border-red-200 rounded-lg p-4">
										<div className="flex items-center">
											...
											<span className="text-red-700 font-medium">This payment link has expired</span>
										</div>
									</div>
								)}
							</CardContent>
										...
										{hasInsufficientBalance && (
											<div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
												<div className="flex items-center">
													...
													<span className="text-yellow-700">Insufficient USDC balance. You need {state.paymentLink.amount} USDC.</span>
												</div>
											</div>
										)}
									</div>
					...
					<div className="xl:col-span-1">
						<Card className="sticky top-8">
							...
							<CardContent className="space-y-4">
								...
								<Button className="w-full" size="lg" disabled={!isConnected || isExpired || hasInsufficientBalance || state.isProcessing} onClick={handlePayment}>
									{state.isProcessing ? (
										<>
								...
								<div className="text-xs text-muted-foreground text-center">Powered by x402 • Secure USDC payments</div>
							</CardContent>
						</Card>
					</div>
				</div>
